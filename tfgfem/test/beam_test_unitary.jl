...
PrintParaview(v_nodes, v_mesh, "beam_mesh_unitary.vtu")
...
dimension2 = [1,0,1]
pos_BC2 = [mesh_distribution[1][end], 0.0, mesh_distribution[3][end]]
...
"""RUNNING THE FINITE ELEMENT ANALYSIS"""
 (voigtstrains_el, voigtstresses_el, strains_nod, stresses_nod, strains_el, stresses_el) = FEAnalysis(v_nodes, v_mesh, E_mesh, pois_mesh, u, f, restricted_DOF)
nodal_connectivity = NodalConnectivity(v_nodes, v_mesh)
E_nodes = NodalYoungModule(nodal_connectivity, E_mesh)
PrintParaview(v_nodes, v_mesh, strains_nod, stresses_nod, u, E_nodes, "beam_analysis_unitary.vtu")
